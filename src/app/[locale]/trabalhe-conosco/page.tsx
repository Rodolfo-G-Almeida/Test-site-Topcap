...
function TrabalheConosco() {
  ...
  const onSubmit: SubmitHandler<Inputs> = async (data) => {
    const objToSend = {
      ...
      html: `<p>Dados enviados:</p><p>Nome: ${data.nome}</p><p>Celular: ${data.celular}</p><p>Email: ${data.email}</p><p>Link Currículo: ${data.link}</p><p>Mensagem: ${data.mensagem}</p>`,
    };
    setSending(true);
  ...
  return (
    <BaseLayout>
      <HeaderPage title={t("menu14")} subtitle="" />
      <div className="box-contato-page">
        <div className="info-contato">
          <h4>RH</h4>
          <h5>rh@topcau.com.br</h5>
        </div>
        <div className="form-contato">
          <form onSubmit={handleSubmit(onSubmit)}>
            ...
            <div className="box-campo">
              <p>
                <label>{tForm("linkResume")}</label>
                <br />
                <input
                  placeholder={tForm("linkCv")}
                  type="text"
                  {...register("link")}
                  aria-invalid={errors.email ? "true" : "false"}
                />
